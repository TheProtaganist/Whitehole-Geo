...
import whitehole.ai.AICommandPanel;
...
public class GalaxyEditorForm extends javax.swing.JFrame {
    ...
    public javax.swing.JPanel aiCommandPanel; // Changed to JPanel to allow fallback
    ...
    private void initGUI() {
        ...
        // Initialize AI Command Panel
        try {
            aiCommandPanel = new AICommandPanel(this);
        } catch (Exception e) {
            System.err.println("[ERROR] Failed to create AI Command Panel: " + e.getMessage());
            e.printStackTrace();
            // Create a fallback panel with error message
            aiCommandPanel = createFallbackAIPanel(e);
        }
        
        // Add AI Commands tab now that the panel is ready
        if (aiCommandPanel != null) {
            tabData.addTab("AI Commands", aiCommandPanel);
        }
    ...
    /**
     * Helper method for AI system to create objects
     * @param objectType The type of object to create (e.g., "Goomba", "Coin", etc.)
     * @param position The position where to place the object
     * @return The created object, or null if creation failed
     */
    public AbstractObj addObjectFromAI(String objectType, Vec3f position) {
        try {
            // Default to common layer and objinfo type for most objects
            String layer = "common";
            String objinfo = "objinfo";
            
            // Create the object based on the whitehole system
            AbstractObj newobj = null;
            StageArchive destZoneArc = curZoneArc;
            
            // Use the existing object creation logic
            newobj = new LevelObj(destZoneArc, layer, objectType, position);
            ...
            // Set object ID
            newobj.data.put("l_id", generateID(objinfo));
            newobj.uniqueID = uniqueID;
            
            // Add to zone archive
            destZoneArc.objects.get(layer).add(newobj);
            
            // Add to all forms
            TreeNode newNode = addAbstractObjToAllForms(uniqueID, objinfo, newobj);
            
            // Update rendering
            String renderKey = String.format("addobj:%1$d", uniqueID);
            addRerenderTask(renderKey);
            ...
            System.err.println("Failed to create object of type " + objectType + ": " + e.getMessage());
            e.printStackTrace();
            return null;
        }
    }
    ...
    private class GalaxyRenderer implements GLEventListener, MouseListener, MouseMotionListener, MouseWheelListener, KeyListener {
        ...
        public void mouseDragged(MouseEvent e) {
            ...
            if(!selectedObjs.isEmpty() && selectedObjs.containsKey(underCursor >>> 3)) {
                ...
                // Camera rotation: Right-click OR Shift+Left-click (based on settings)
                boolean shouldRotateCamera = false;
                
                if (Settings.getUseRightClickCamera()) {
                    // Traditional mode: right-click rotates camera
                    shouldRotateCamera = (mouseButton == MouseEvent.BUTTON3);
                } else {
                    // New default mode: Shift+left-click rotates camera, right-click does other actions
                    shouldRotateCamera = (mouseButton == MouseEvent.BUTTON1 && e.isShiftDown());
                }
                ...
                else if(mouseButton == MouseEvent.BUTTON1 && !e.isShiftDown()) {
...
//            }
    ...
    private void initComponents() {
        ...
        // AI Commands tab will be added later when aiCommandPanel is initialized
    ...
    /**
     * Creates a fallback AI panel when the main AICommandPanel fails to initialize
     */
    private javax.swing.JPanel createFallbackAIPanel(Exception error) {
        javax.swing.JPanel fallbackPanel = new javax.swing.JPanel(new java.awt.BorderLayout());
        fallbackPanel.setBorder(javax.swing.BorderFactory.createTitledBorder("AI Commands (Error)"));
        
        javax.swing.JTextArea errorText = new javax.swing.JTextArea();
        errorText.setEditable(false);
        errorText.setFont(new java.awt.Font(java.awt.Font.MONOSPACED, java.awt.Font.PLAIN, 12));
        errorText.setText("AI Commands failed to initialize.\n\n" + 
                         "Error: " + error.getMessage() + "\n\n" +
                         "This might be due to:\n" +
                         "• Missing AI dependencies\n" +
                         "• Corrupted AI configuration\n" +
                         "• System compatibility issues\n\n" +
                         "Please check the console output for detailed error information.\n" +
                         "You can still use all other Whitehole features normally.");
        
        javax.swing.JScrollPane scrollPane = new javax.swing.JScrollPane(errorText);
        fallbackPanel.add(scrollPane, java.awt.BorderLayout.CENTER);
        
        return fallbackPanel;
    }
    
    /**
     * Helper method to safely add objects from AI (if AI panel is working)
     */
    public void addObjectFromAI(AbstractObj obj) {
        if (aiCommandPanel instanceof AICommandPanel) {
            // AI panel is working, can use advanced features
            addAbstractObjToAllForms(obj.uniqueID, obj.name, obj);
        } else {
            // Fallback mode, just add the object normally
            addAbstractObjToAllForms(obj.uniqueID, obj.name, obj);
        }
    }
